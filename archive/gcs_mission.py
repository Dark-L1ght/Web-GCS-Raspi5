...
def stop_mission_and_land():
    ...
    # Command to find the process ID of gps_mission.py and kill it forcefully
    kill_command = "pgrep -f gps_mission.py | xargs kill -9"
            ...
            try:
                ...
                if action == "start_mission":
                    ...
                    if not waypoints or len(waypoints) != 8:
                        print(
                            f"Mission start aborted: Invalid waypoints. Expected 8, received {len(waypoints) if waypoints else 0}."
                        )
                    ...
                    script_path = (
                        "/home/kingphoenix/Web-GCS-Raspi5/scripts/run_mission.sh"
                    )
                    mission_command = f"{script_path} '{waypoints_json_str}'"

                    asyncio.create_task(
                        asyncio.to_thread(execute_ssh_command, mission_command)
                    )
