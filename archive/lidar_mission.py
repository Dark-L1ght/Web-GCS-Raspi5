...
CENTERING_ERROR_THRESHOLD = (
    0.50  # Normalized error (50%) at which descent speed hits minimum.
)
...
FINAL_VERIFICATION_THRESHOLD = (
    0.15  # A tighter centering tolerance (15%) for the final check
)
...
class ArduinoLidarReader:
    ...
    def __init__(self, port, baudrate):
        ...
        self.thread = None
        print(
            f"ArduinoLidarReader initialized for port {self.port} at {self.baudrate} baud."
        )
    ...
    def _read_thread(self):
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                print(
                    f"Error: Could not connect to Arduino on {self.port}. Retrying in 5 seconds..."
                )
                time.sleep(5)
            finally:
...
class VelocityController:
    ...
    def __init__(self, p_gain, d_gain):
        ...
        print(
            f"PD VelocityController initialized with P={self.p_gain}, D={self.d_gain}"
        )
...
def send_control_command(command):
    try:
        ...
        print(
            f"Error: Connection refused. Is the detection script running on port {CONTROL_SERVER_PORT}?"
        )
...
def land_normally(master):
    ...
    try:
        ...
        print(
            f"Warning: Did not receive disarm confirmation, but landing was commanded. {e}"
        )
    ...
    while time.time() - verification_start_time < FINAL_VERIFICATION_TIMEOUT:
        try:
            ...
            if final_error_ratio < FINAL_VERIFICATION_THRESHOLD:
                ...
                print(
                    f"Verification FAILED. Target drifted. Final error: {final_error_ratio:.1%}"
                )
                return False
...
def center_above_target(master, sock, target_alt, drop_type="dual"):
    ...
    print(
        f"Centering for '{drop_type}' drop (Accepting IDs: [0, 1]) at {target_alt:.2f}m..."
    )
    ...
    while time.time() - start_time < CENTERING_TIMEOUT:
        ...
        try:
            ...
            is_horizontally_centered = (
                horizontal_error_ratio < CENTERING_SUCCESS_THRESHOLD
            )
            ...
            print(
                f"\rCENTERING (ID {detected_id}): Alt: {current_altitude:.2f}m, H-Err: {horizontal_error_ratio:.1%}, V-Err: {vertical_error_ratio:.1%}, Area: {area_progress_ratio:.1%}",
                end="",
            )
...
def execute_precision_landing(master, sock, pickup_type):
    ...
    send_control_command("resume")
    print(
        f"Starting precision landing for '{pickup_type}' pickup (Accepting IDs: [0, 1])..."
    )
    ...
    while time.time() - search_start_time < LANDING_TIMEOUT:
        ...
        try:
            ...
            print(
                f"\rLANDING (ID {detected_id}): Alt: {current_altitude:.2f}m, Gain: {horizontal_gain:.2f}, Err: {center_error_ratio:.2%}, Vz: {dynamic_down_vel:.2f} m/s",
                end="",
            )
            ...
            if time_since_lost < TARGET_LOST_HOVER_DURATION:
                print(
                    f"\rTarget lost. Hovering... (Time since last seen: {time_since_lost:.1f}s)",
                    end="",
                )
            else:
...
def handle_window_approach(master, target_altitude):
    ...
    print(
        f"Target Alt: {target_altitude:.2f}m | GPS Alt: {current_gps_alt:.2f}m | Error: {altitude_error:.2f}m"
    )
...
def strafe_until_distance(master, lidar_manager, direction, target_distance):
    ...
    while time.time() - start_time < TIMEOUT:
        ...
        sys.stdout.write(
            f"\rStrafing {direction}... Side Dist: {side_dist:.2f}m, Fwd Vel: {fwd_vel:.2f}m/s"
        )
        sys.stdout.flush()
...
def navigate_sideways_to_target(
    master, lidar_manager, sock, direction, target_class_id
):
    print(
        f"--- Navigating sideways ({direction}) to find target ID {target_class_id} ---"
    )
    flush_socket_buffer(sock)
    send_control_command("resume")
    TIMEOUT = 20
    ...
    while time.time() - start_time < TIMEOUT:
        try:
            ...
            detection = json.loads(data.decode())
            if (
                detection.get("state") == "TRACKING"
                and detection.get("class_id") == target_class_id
            ):
                print(
        ...
        fwd_vel = -CENTERING_GAIN * error
        sys.stdout.write(
            f"\rStrafing {direction}... Wall Dist: {front_dist:.2f}m, Fwd Vel: {fwd_vel:.2f} m/s"
        )
        sys.stdout.flush()
        master.mav.send(
            mavutil.mavlink.MAVLink_set_position_target_local_ned_message(
...
def follow_corridor(master, lidar_manager, fwd_speed, stop_condition_func):
    ...
    while time.time() - start_time < TIMEOUT:
        ...
        centering_error = (
            left_dist - right_dist if not (left_dist > 5 or right_dist > 5) else 0
        )
        right_vel = -CENTERING_GAIN * centering_error
        sys.stdout.write(
            f"\rFollowing... Fwd: {fwd_speed:.2f}, Right Vel: {right_vel:.2f}, Front: {distances['front']:.2f}m"
        )
        sys.stdout.flush()
        master.mav.send(
            mavutil.mavlink.MAVLink_set_position_target_local_ned_message(
...
def main():
    ...
    if len(sys.argv) > 1:
        try:
            ...
            if len(waypoints_from_gcs) == 8:
                ...
                print(
                    f"ERROR: Expected 8 waypoints, got {len(waypoints_from_gcs)}. Using defaults."
                )
        except Exception as e:
    ...
    lidar_manager = ArduinoLidarReader(
        port=ARDUINO_LIDAR_PORT, baudrate=ARDUINO_LIDAR_BAUDRATE
    )
    lidar_manager.start()
    ...
    try:
        ...
        if not follow_corridor(
            master, lidar_manager, CORRIDOR_FWD_SPEED, stop_at_logistic_1
        ):
        ...
        if not follow_corridor(
            master, lidar_manager, CORRIDOR_FWD_SPEED, stop_at_corner
        ):
        ...
        distances = lidar_manager.get_distances()
        opening_direction = (
            "left" if distances.get("left", 0) > distances.get("right", 0) else "right"
        )
        ...
        if not center_above_target(
            master, sock=data_sock, target_alt=TAKEOFF_ALTITUDE, drop_type="dual"
        ):
